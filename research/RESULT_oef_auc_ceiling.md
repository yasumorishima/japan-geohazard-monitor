...
>
> 🔴 **Correction 2026-08-21.** The title of this document says 0.9 is hollow, and that is now too broad to stand as written. What was hollow was one specific arm: a half-degree grid that reached 0.9011 with a *negative* Brier skill, and the same thing reproduced deliberately on 2026-08-21 at 0.9148 with a skill of only +0.0133, where splitting the Brier score shows the resolution is 3.12 per cent of the uncertainty against 6.94 at one degree -- a finer grid makes negatives easier, which lifts an AUC and cannot lift a skill score. The one-degree arm is not that: on a 43-year catalogue over 38 annual windows it reads a pooled AUC of 0.9010 with a Brier skill of +0.0608, and combining it with a background-only arm under weights fitted on earlier windows reads 0.9036 with +0.0694. Since 2026-08-20 the target has been judged against a bar fixed before each run: the 2.5th percentile of the pooled AUC over 2000 resamples of the windows must exceed 0.90 **and** every resample must keep the Brier skill positive. The skill half is met in all 2000 resamples; the percentile stands at 0.8985 and the bar is not. Separately, an honest ceiling for any time-invariant score -- the per-cell rate learned on interleaved windows and scored on the others -- measures 0.8956, which the arm above already exceeds, so no ceiling is claimed here for the problem, only for that one axis.
>
