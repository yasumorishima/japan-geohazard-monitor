...
## Detection threshold (synthetic injection)

To turn the null into a quantitative bound, synthetic slow slip (a known-megathrust Okada thrust dislocation x a 0->1 window ramp) was injected into the real residuals, and the minimum slip S* at which the fixed-geometry trajectory detector's variance reduction crosses the baseline noise floor (95th-percentile VR = 0.27) was found by bisection. Result: S* = 0.12 m across baseline noise realizations (median) and 0.41 m in the actual Iquique window - a uniform thrust slip of seismic moment **Mw ~6.5 (baseline median) to ~6.9 (actual window)** on the 50x40 km megathrust patch. The detection floor therefore sits right at Ruiz et al. (2014)'s ~Mw 6.5 estimate for the precursory slow slip: the documented signal is at or below what this reproducible pipeline can resolve, which is precisely why the positive control returns null. (Mw scales with the assumed patch area; the floor is quoted for a uniform-slip patch of this size.)
