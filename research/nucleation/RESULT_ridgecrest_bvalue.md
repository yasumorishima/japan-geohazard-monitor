...
### B3 — spatial low-b asperity not resolvable (location-permutation control)
PyOcto association (1-D SoCal model, Vp 6.0 / Vs 3.45 km/s) located 389 events (depth p10-p90 = 7.1-15.2 km). A nearest-neighbour b-positive field (K = 50 / 80 events per patch) puts the M7.1 nucleation patch near the field median (M7.1-patch b+ = 0.67; field median 0.65-0.68; percentile 0.41 at K=50, 0.58 at K=80 — not in the low-b quartile, robust across K). BUT a location-permutation control shows the field is statistically FLAT: the observed field dispersion (std = 0.061) is indistinguishable from a location-shuffled null (mean 0.060, p10-p90 0.055-0.065; p(null >= obs) = 0.35). So the b-positive field is NOT spatially resolved at this catalogue size (389 located events, network-relative magnitudes), and the percentile is ranking noise rather than real spatial b-structure. The honest B3 statement is that no low-b asperity is spatially resolvable here — consistent with both no asperity and an unresolved field, and with Kumamoto B3 where the apparent low-b contrast did not survive a prospective blind ranking. (Absolute b+ ~0.67 here is not comparable to the temporal-test b+ ~0.9: different event subset, K-neighbour windows, and relative-magnitude zero-point.)

## Conclusion
With independent network magnitudes, Ridgecrest shows **no prospective foreshock b-value drop before the M7.1**: the incompleteness-robust b-positive stays near ~0.9 through the final approach (final-12 h / 6 h / 3 h = 0.96 / 0.97 / 0.93) rather than declining, while the low maximum-curvature value (0.79) is attributable to transient post-mainshock incompleteness — so the apparent FTLS RED light is not reproduced at independent-magnitude resolution, consistent with the parameter-sensitivity reported by Dascher-Cousineau et al. (2020). The spatial low-b-asperity channel (B3, PyOcto-located, 389 events) is also null: the b-positive field is statistically flat (dispersion indistinguishable from a location-shuffled null, p = 0.35), so no low-b asperity is spatially resolvable at this catalogue size. The b-value channel is thus B1/B2/B3 complete and uniformly null, fully apples-to-apples with Kumamoto. This is the second case (n = 2) on the b-value channel; both Kumamoto (M6.5 to M7.3) and Ridgecrest (M6.4 to M7.1) are null on a prospective temporal b-value precursor, the latter being Gulia & Wiemer's own real-time showcase.
