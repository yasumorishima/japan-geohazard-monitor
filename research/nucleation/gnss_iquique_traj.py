"""Iquique positive control - HIGH-SENSITIVITY trajectory variant. The
net-displacement tests (endpoint medians) failed the positive control. This
replaces the endpoint-difference statistic with a full-window linear-trajectory
fit (least-squares slope over ALL epochs x window length = the matched filter for
a ~linearly growing slow-slip transient), recovering ~sqrt(N_epoch) of stacking
gain, then feeds it to the same grid-searched Okada megathrust inversion with
baseline-window control. If Ruiz et al. (2014)'s slow slip is recoverable at NGL
5-min precision at all, the trajectory matched filter should expose it: window
VR > baseline 95th percentile = POSITIVE (method validated).
"""
...
def best_vr(getobs):
    ...
    for lo0,la0,dep,strike,dip in GEOMS:
        ...
        if vr>best: best=vr
    return best
vr_win=best_vr(lambda s: ramp_disp(*R[s],WIN_START,WIN_END))
base=[];a=BASE_START
while a+WINLEN<=BASE_END:
    aa=a; v=best_vr(lambda s: ramp_disp(*R[s],aa,aa+WINLEN))
...
print(json.dumps(dict(stat="ramp_trajectory",n_fit=len(fit_st),vr_win=float(vr_win) if vr_win is not None else None,
