...
The same injection characterization run at Kumamoto: synthetic Futagawa strike-slip (Okada x window ramp) added to the real residuals, the minimum slip S* at which the fixed-geometry trajectory detector crosses the baseline noise floor (95th-percentile VR = 0.067, far below Iquique's 0.27 thanks to 35 onshore stations at 5-60 km) found by bisection. Result: S* = 0.08 m (baseline median) to 0.25 m (actual window) on a 14x10 km patch = **Mw ~5.6 to ~5.9** - about one magnitude unit more sensitive than Iquique (Mw ~6.5-6.9), the gain coming entirely from the dense near-fault onshore geometry. The Kumamoto precursory slow slip - inferred from seismicity (Kato et al. 2016) and never resolved geodetically - lies below this Mw ~5.9 bound. So across both cases the geodetic null is a quantified detectability limit (Mw ~5.6-6.9 depending on network geometry), with the documented/inferred precursors sitting beneath it.
