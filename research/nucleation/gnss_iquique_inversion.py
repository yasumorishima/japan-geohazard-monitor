"""Iquique 2014 positive control - forward slip-model inversion (the decisive
test; controls against baseline windows, not against the non-quiet central-Chile
far stations). Fits an offshore megathrust elastic dislocation (validated
Okada85) to the 16-day foreshock-window net-displacement field and asks whether
the best grid-searched variance reduction beats the same search on quiet
baseline windows. If Ruiz et al. (2014)'s slow slip is recoverable at NGL 5-min
precision, the window VR should exceed the baseline VR distribution (POSITIVE),
validating the method and confirming the Kumamoto geodetic null is a true absence.
"""
...
EPI=(-19.61,-70.77); SIG_MAX=0.05; RESID_CAP=0.5; FIT_KM=250.0; EDGE=86400.0
...
def netdisp(T,rE,rN,a,b):
    ...
    Tn,e,n=T[m],rE[m],rN[m]; f=Tn<=a+EDGE;l=Tn>=b-EDGE
...
LON0=[-71.3,-71.0,-70.7]; LAT0=[-19.9,-19.6,-19.3]; DEP=[15.0,25.0,35.0]; STR=[355.0,10.0]; DIP=[15.0,20.0]
L=50.0; W=40.0
...
def best_vr(getobs):
    ...
    for lo0,la0,dep,strike,dip in GEOMS:
        ...
        vr=1.0-np.sum((ov-A@coef)**2)/sst
        if vr>best: best=vr
    return best
vr_win=best_vr(lambda s: netdisp(*R[s],WIN_START,WIN_END))
base=[];a=BASE_START
while a+WINLEN<=BASE_END:
    aa=a; v=best_vr(lambda s: netdisp(*R[s],aa,aa+WINLEN))
...
print(json.dumps(dict(n_fit=len(fit_st),vr_win=float(vr_win) if vr_win is not None else None,
    base_vr_p95=p95, base_vr_med=float(np.median(base)) if len(base) else None, n_base=len(base),
