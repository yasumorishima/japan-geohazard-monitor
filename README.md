...
- **Prospective result (2026-06-10, full window to ~45 min before the M9)**: the void-filled micro-catalogue (5,385 SAC over 03-09 00:00 → 03-11 14:00 JST, 49 stations, 10.7K PhaseNet picks, 87 PyOcto-located events) reproduces the documented foreshock migration toward the eventual M9 hypocentre through ~14 h before the mainshock — median distance 244 → 78 km and the near-front (10th-percentile) distance 157 → 33 km by 03-11 00:00 JST, the fraction of events within 75 km rising 0 → 0.45, and the rate accelerating (corr(time, rate) = +0.50, peaking at 3.3× on 03-10 18:00) — prospectively, from raw waveforms with no catalogue input (cf. Kato et al. 2012).
...
- **The final ~14 h is not resolved (method limit, not a physical null)**: the last two 6-h bins (03-11 06:00 / 12:00 JST) regress to 258 / 273 km with sparse association (n = 3 / 5) and locate onshore — the M9 nucleation patch sits ~180 km offshore, at the edge of the onshore Hi-net array's resolving power, and a homogeneous 0-D velocity model breaks down beyond ~200 km (Pn). The whole-window linear corr(time, distance) = −0.14 is therefore dominated by that tail; the genuine prospective signal is the migration + acceleration up to ~14 h before the mainshock. Closing the immediate pre-mainshock gap needs a 1-D/offshore velocity model or template/cross-correlation relocation (as Kato 2012 used), not naive association.
