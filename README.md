...
| 階層 | 場所 | 役割 | 容量 | 保持 |
|---|---|---|---|---|
| Canonical (durable) | **Hugging Face dataset** [`yasumorishima/japan-geohazard`](https://huggingface.co/datasets/yasumorishima/japan-geohazard) `geohazard.db` | merge job が `scripts/hf_sync.py` で upload (integrity_check fail-closed + no-shrink guard で degraded 上書き防止 + `super_squash_history` で LFS 履歴を畳む) | LFS 16.5 GB 想定 | 永続 (public dataset) |
