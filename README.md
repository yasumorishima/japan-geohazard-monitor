...
- **First prospective result (2026-06-10)**: on the verified part of the window (03-09 00:00 → 03-10 08:00 JST, 49 stations, ~11.4K picks), the auto-built micro-catalogue shows the foreshock sequence migrating toward the eventual M9 hypocentre — median distance 228 → 53 km, corr(time, distance) = −0.92, fraction of events within 75 km rising monotonically 0 → 1.0 — reproducing Kato et al. (2012) from raw waveforms with no catalogue input.
- **Data-quality caveat being remediated**: the final ~28 h before the M9 were fetched while the production Hi-net backfill cron raced the research fetch and mutated the account-global station selection, so only 3 in-network stations cover that span (association becomes impossible; picks themselves are abundant). Distant stations cannot substitute — a homogeneous 0-D velocity model breaks beyond ~200 km (Pn). Fix shipped: the fetch script now re-asserts the station selection before every request, and the affected 30 h are being re-fetched; the final pre-M9 verdict (migration + rate acceleration up to 2.8 h before the mainshock) is pending that clean re-fetch.
