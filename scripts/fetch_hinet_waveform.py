"""Fetch Hi-net waveform features from NIED — Phase 2 (3) snet_waveform 統合.

Hi-net (High-sensitivity Seismograph Network of Japan) is NIED's nationwide
high-sensitivity short-period seismometer network, with ~800 borehole-installed
stations across Japan equipped with 1 Hz natural-period 3-component velocity
sensors, sampled at 100 Hz.

Network code: 0101 (single sensor type — short-period high-sensitivity velocity).
...
snet_waveform for cross-source ML feature parity):
    ...
    - VLF columns kept NULL (Hi-net's 1 Hz natural period cannot resolve
      0.01–0.1 Hz reliably).

Backfill strategy:
    - Initial 15 stations geographically distributed (env override HINET_STATIONS)
    - Recent 7 days (4 segments/day) + backfill gaps (1 segment/day)
    - HinetPy ~200 req/day quota is per NIED account (session cookie),
      shared with fetch-snet (default 144 req/cron). Hi-net default budget
      is kept low (30 req/cron) so total stays under quota.
      Override via HINET_MAX_REQUESTS Repository Variable.
    - Reports per-region coverage via Discord

References:
    Okada et al. (2004) Earth Planets Space 56 (Hi-net description)
    Obara (2002) Nonvolcanic deep tremor — Hi-net first detection
    Ide et al. (2007) Slow earthquakes — Hi-net long-period waveforms
"""
...
HINET_NETWORK_CODE = "0101"
HINET_SENSOR_TYPE = "high_sensitivity"
# Backfill window starts at NIED Hi-net data archive epoch (2004-04-01),
# the earliest reliably retrievable date via HinetPy continuous waveform API.
HINET_START_STR = "2004-04-01"

# HinetPy constraints — shared NIED account quota with S-net.
# Split: snet 60 + hinet 60 = 120 per run (out of NIED ~150 daily budget).
...
MAX_REQUESTS_PER_RUN = int(os.environ.get("HINET_MAX_REQUESTS", "30"))
...
STEP_BUDGET_SEC = int(os.environ.get("HINET_STEP_BUDGET_SEC", "4200"))
DEADLINE_MARGIN_SEC = int(os.environ.get("HINET_DEADLINE_MARGIN_SEC", "300"))
...
MAX_ACTIVE_STATIONS = int(os.environ.get("HINET_MAX_STATIONS", "30"))
...
HINET_STATIONS_ENV = os.environ.get("HINET_STATIONS", "").strip()

# Hi-net geographic regions for spatial analysis (instead of S-net cable segments).
...
HINET_REGIONS = {
    ...
    "H-HKD":   {"lat_range": (41.5, 46.0), "lon_range": (139.5, 146.0), "desc": "Hokkaido",          "order": 0},
    "H-TOH-N": {"lat_range": (39.5, 41.5), "lon_range": (139.0, 142.5), "desc": "Tohoku North",      "order": 1},
    "H-TOH-S": {"lat_range": (37.0, 39.5), "lon_range": (139.0, 142.0), "desc": "Tohoku South",      "order": 2},
    "H-KNT":   {"lat_range": (35.0, 37.0), "lon_range": (138.5, 141.5), "desc": "Kanto",             "order": 3},
    "H-IZU":   {"lat_range": (28.0, 35.0), "lon_range": (138.5, 142.5), "desc": "Izu/Bonin",         "order": 4},
    "H-CHB":   {"lat_range": (35.5, 37.5), "lon_range": (135.5, 138.5), "desc": "Chubu",             "order": 5},
    "H-KSI":   {"lat_range": (33.5, 35.5), "lon_range": (134.5, 137.0), "desc": "Kansai",            "order": 6},
    "H-CHG":   {"lat_range": (33.5, 36.0), "lon_range": (130.5, 134.5), "desc": "Chugoku",           "order": 7},
    "H-SKK":   {"lat_range": (32.5, 34.5), "lon_range": (132.5, 135.0), "desc": "Shikoku",           "order": 8},
    "H-KYU-N": {"lat_range": (32.0, 34.0), "lon_range": (129.5, 132.5), "desc": "Kyushu North",      "order": 9},
    "H-KYU-S": {"lat_range": (30.5, 32.5), "lon_range": (129.5, 132.0), "desc": "Kyushu South",      "order": 10},
    "H-OKN":   {"lat_range": (24.0, 28.0), "lon_range": (122.0, 131.0), "desc": "Okinawa/Ryukyu",    "order": 11},
}
...
# FFT window size — Hi-net is short-period 1 Hz, FFT_WINDOW_SEC matches snet/fnet for parity.
FFT_WINDOW_SEC = 10
...
TABLE_DDL = """
CREATE TABLE IF NOT EXISTS hinet_waveform (
...
INDEX_DDL = [
    "CREATE INDEX IF NOT EXISTS idx_hinet_wf_date ON hinet_waveform(date_str);",
    "CREATE INDEX IF NOT EXISTS idx_hinet_wf_station ON hinet_waveform(station_id);",
    "CREATE INDEX IF NOT EXISTS idx_hinet_wf_region ON hinet_waveform(region);",
]

FAILED_DATES_DDL = (
    "CREATE TABLE IF NOT EXISTS hinet_failed_dates ("
...
async def ensure_table(db: aiosqlite.Connection) -> None:
    """Create hinet_waveform + hinet_failed_dates tables and indexes."""
...
async def get_existing_dates(db: aiosqlite.Connection) -> set[str]:
    """Return set of date_str values already in hinet_waveform."""
    rows = await db.execute_fetchall(
        "SELECT DISTINCT date_str FROM hinet_waveform"
    )
...
async def get_failed_dates(
    ...
    coincided with consecutive runs), reset its retry_count:
        ...
        UPDATE hinet_failed_dates SET retry_count = 0 WHERE date_str = '2018-09-15';

    or wipe the marker entirely:

        DELETE FROM hinet_failed_dates WHERE date_str = '2018-09-15';
    """
    cutoff_iso = (
        datetime.now(timezone.utc) - timedelta(days=FAILED_DATES_RETRY_AFTER_DAYS)
    ).isoformat()
    rows = await db.execute_fetchall(
        "SELECT date_str FROM hinet_failed_dates "
...
def _mark_failed_sync(conn, date_str: str, reason: str = "no_records") -> None:
    ...
    conn.execute(
        "INSERT OR IGNORE INTO hinet_failed_dates "
        "(date_str, last_failed_at, retry_count, reason) "
        "VALUES (?, ?, 0, ?)",
        (date_str, now_str, reason),
    )
    conn.execute(
        "UPDATE hinet_failed_dates "
...
def compute_waveform_features(data_z, data_x, data_y, fs: float) -> dict | None:
    ...
    Hi-net is short-period (1 Hz natural period); VLF columns are
    always returned as None for schema parity with snet/fnet.
    """
    import numpy as np
    ...
    # VLF (0.01–0.1 Hz) intentionally skipped: Hi-net 1 Hz natural-period
    # short-period sensor cannot reliably resolve that band, so vlf_power /
    # vlf_hv_ratio remain None as initialized (schema parity with snet/fnet).
...
def select_active_stations(client, max_stations: int) -> tuple[list[str], dict]:
    """Select Hi-net stations for fetching.

    Returns (station_codes, station_coords_map). When HINET_STATIONS env is
    ...
    """
    try:
        all_stations = client.get_station_list(HINET_NETWORK_CODE)
    except Exception as exc:
        logger.warning("Hi-net station list fetch failed: %s", exc)
        return [], {}
    ...
    logger.info("Hi-net network reports %d stations total", len(coords_map))

    if HINET_STATIONS_ENV.upper() == "ALL":
        logger.info("HINET_STATIONS=ALL → no station filter")
        return list(coords_map.keys()), coords_map

    if HINET_STATIONS_ENV:
        explicit = [s.strip() for s in HINET_STATIONS_ENV.split(",") if s.strip()]
        logger.info("HINET_STATIONS env override: %d explicit stations", len(explicit))
...
def _fetch_day(
    ...
    """Fetch and process one day's Hi-net waveform data.
    ...
    for hour in segment_hours:
        ...
        work_dir = tempfile.mkdtemp(prefix=f"hinet_{date_str}_{hour:02d}_")
        ...
        try:
            ...
            data = client.get_continuous_waveform(
                HINET_NETWORK_CODE, start, REQUEST_DURATION_MIN, outdir=work_dir,
            )
            ...
            for sac_path in sac_files:
                ...
                # Hi-net SAC channel is 1-char per HinetPy convention
                # (U/N/E), but win2sac_32 may emit 2-char UD/NS/EW depending
                # on the channel-table mapping. Accept both forms defensively.
                # All Hi-net components are 100 Hz (no long-period branch).
            ...
            for station_id, comps in station_files.items():
                ...
                # fs consistency check: defensive — Hi-net is uniformly 100 Hz,
                # but reject any triplet with mismatched fs to keep PSD inputs sane.
...
def _save_records_sync(conn, records: list[dict], now_str: str) -> tuple[int, int]:
    """Insert records into hinet_waveform with INSERT OR IGNORE.
    ...
    for rec in records:
        try:
            cursor = conn.execute(
                """INSERT OR IGNORE INTO hinet_waveform
...
def _fetch_and_save(
    ...
    if selected and HINET_STATIONS_ENV.upper() != "ALL":
        ...
        try:
            client.select_stations(HINET_NETWORK_CODE, selected)
            ...
            try:
                ...
                client.select_stations(HINET_NETWORK_CODE, stripped)
...
async def main() -> None:
    """Fetch Hi-net waveform features with incremental backfill."""
    credentials = _check_credentials()
    if credentials is None:
        logger.warning(
            "HINET_USER/HINET_PASS not set. Hi-net waveform fetch requires "
            "NIED Hi-net registration. Exiting gracefully."
        )
        return

    user, password = credentials
    logger.info("Starting Hi-net short-period waveform extraction (Phase 2 (3))")
    logger.info("Network code: %s, max active stations: %d, max requests: %d",
                HINET_NETWORK_CODE, MAX_ACTIVE_STATIONS, MAX_REQUESTS_PER_RUN)
    ...
    hinet_start = datetime.strptime(HINET_START_STR, "%Y-%m-%d")
    ...
    while current >= hinet_start and len(backfill_dates) < MAX_BACKFILL_DAYS_PER_RUN:
    ...
    coverage_path = results_dir / "hinet_coverage.json"
...
def _log_coverage_report(report: dict) -> None:
    ...
    logger.info("=== Hi-net Waveform Coverage ===")
