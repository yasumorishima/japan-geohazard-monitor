"""Smoke test for Phase 2 Hi-net backfill acceleration.
...
class TestPhase2Constants(unittest.TestCase):
    def test_max_backfill_days_per_run_raised(self):
        import fetch_hinet_waveform as f
    ...
    def test_old_failed_date_rolls_back_into_fetch_pool(self):
        ...
        async def run():
            async with aiosqlite.connect(":memory:") as db:
                ...
                await db.execute(
                    "INSERT INTO hinet_failed_dates (date_str, last_failed_at, retry_count, reason) "
                    "VALUES (?, ?, ?, ?)",
